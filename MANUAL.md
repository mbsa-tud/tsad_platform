...
---
**NOTE: In this manual, numbers in round parantheses (E.g. (1)) always reference a section in an image**
...
* Click `Quick Load all Models` to load a default configuration of all implemented models. **NOTE** The default configuration for non-deep-learning models has subsequences disabled. This means that they detect anomalous point's rather than sequences. The use of subsequences can be enabled when adding models manually (see next option).
* Click `Add Model Manually` to configure models by hand. This opens a **new window**, allowing you to select a model and configure its parameters. Several different deep-learning and machine-learning based models are available. Once configured, click `Add to Model Selection` to add the selected model to the list of models.
