function anomalyScores = run_MERLIN(data, minL, maxL, k)
%RUNMERLIN
%
% Adaption of MERLIN to output k discords

r = 5;
display_meta_info = false;
numLengths = maxL - minL + 1;
disc_dist_a = zeros(numLengths, k);
disc_loc_a = zeros(numLengths, k);
lengths = minL:maxL;
for lengthIndex = 1:numLengths
    ...
    if length(temp_disc_loc_a) > 1
        [temp_disc_dist_a, idx] = maxk(temp_disc_dist_a, k);
